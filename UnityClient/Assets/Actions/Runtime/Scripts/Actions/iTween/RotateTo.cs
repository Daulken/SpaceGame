...
namespace Actions {
	...
	[AddComponentMenu("SPACEJAM/Actions/Actions/iTween/RotateTo")]
	public class RotateTo : BaseAction
	{
		...
		protected override void RecacheValues()
		{
			...
			m_iTweenArgs.Set("rotation", m_degrees);
		...
		protected override float PerformAction(bool immediate, bool realTime, ref List<System.UInt64> dependantActionHandles)
		{
			...
			if (m_transform == null)
			{
				DebugHelpers.LogWarningContext("RotateTo action on \"{0}\" has no Transform specified", this, gameObject.GetNameWithPath());
				return 0.0f;
			}
			...
			if ((m_duration <= 0.0f) || immediate || !m_transform.gameObject.activeInHierarchy)
			{
				...
				if (m_localSpace)
					m_transform.localRotation = Quaternion.Euler(m_degrees);
				else
					m_transform.rotation = Quaternion.Euler(m_degrees);
				return 0.0f;
			}
			...
			// Execute the iTween
			if (!iTween.RotateTo(m_transform.gameObject, m_iTweenArgs))
				return 0.0f;
