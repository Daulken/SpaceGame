...
namespace Actions {
	...
	[AddComponentMenu("SPACEJAM/Actions/Actions/iTween/MoveTo")]
	public class MoveTo : BaseAction
	{
		...
		protected override void RecacheValues()
		{
			...
			m_iTweenArgs.Set("position", m_vector);
		...
		protected override float PerformAction(bool immediate, bool realTime, ref List<System.UInt64> dependantActionHandles)
		{
			...
			if (m_transform == null)
			{
				DebugHelpers.LogWarningContext("MoveTo action on \"{0}\" has no Transform specified", this, gameObject.GetNameWithPath());
				return 0.0f;
			}
			...
			// Execute the iTween
			if (!iTween.MoveTo(m_transform.gameObject, m_iTweenArgs))
				return 0.0f;
