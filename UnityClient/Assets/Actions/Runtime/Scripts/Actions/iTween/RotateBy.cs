...
namespace Actions {
	...
	[AddComponentMenu("SPACEJAM/Actions/Actions/iTween/RotateBy")]
	public class RotateBy : BaseAction
	{
		public Transform m_transform = null;
		public Vector3 m_degrees = new Vector3(0.0f, 0.0f, 0.0f);
		...
		protected override void RecacheValues()
		{
			...
			m_iTweenArgs.Set("amount", m_degrees);
		...
		protected override float PerformAction(bool immediate, bool realTime, ref List<System.UInt64> dependantActionHandles)
		{
			...
			if (m_transform == null)
			{
				DebugHelpers.LogWarningContext("RotateBy action on \"{0}\" has no Transform specified", this, gameObject.GetNameWithPath());
				return 0.0f;
			}
			...
			{
				// Cancel all Rotate iTween transitions on this game object. Only one Rotate can execute at a time
				iTween.StopByType(m_transform.gameObject, "Rotate", false, true);
				
				m_transform.Rotate(m_degrees, m_localSpace?Space.Self:Space.World);
				return 0.0f;
			}
			...
			// Execute the iTween
			if (!iTween.RotateBy(m_transform.gameObject, m_iTweenArgs))
				return 0.0f;
		...
		protected override void CancelAction(bool actionStillRunning, bool actionHasActiveDependants, bool actionListFinished)
		{
			...
			if (actionStillRunning && (m_transform != null))
				iTween.StopByType(m_transform.gameObject, "Rotate", false, false);
		}
	}
