...
namespace Actions {
	...
	[AddComponentMenu("SPACEJAM/Actions/Actions/iTween/ScaleBy")]
	public class ScaleBy : BaseAction
	{
		public Transform m_transform = null;
		public Vector3 m_scale = new Vector3(0.0f, 0.0f, 0.0f);
		...
		protected override void RecacheValues()
		{
			...
			m_iTweenArgs.Set("amount", m_scale);
		...
		protected override float PerformAction(bool immediate, bool realTime, ref List<System.UInt64> dependantActionHandles)
		{
			...
			if (m_transform == null)
			{
				DebugHelpers.LogWarningContext("ScaleBy action on \"{0}\" has no Transform specified", this, gameObject.GetNameWithPath());
				return 0.0f;
			}
			...
			if ((m_duration <= 0.0f) || immediate || !m_transform.gameObject.activeInHierarchy)
			{
				...
				iTween.StopByType(m_transform.gameObject, "Scale", false, true);
			...
			// Execute the iTween
			if (!iTween.ScaleBy(m_transform.gameObject, m_iTweenArgs))
				return 0.0f;
