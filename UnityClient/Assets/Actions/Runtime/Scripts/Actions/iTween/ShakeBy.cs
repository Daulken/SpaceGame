...
namespace Actions {
	...
	[AddComponentMenu("SPACEJAM/Actions/Actions/iTween/ShakeBy")]
	public class ShakeBy : BaseAction
	{
		public Transform m_transform = null;
		public Vector3 m_vector = new Vector3(0.0f, 0.0f, 0.0f);
		public bool m_localSpace = true;
		public float m_duration = 0.0f;
		public iTween.ShakeType m_shakeType = iTween.ShakeType.shakeRandom;
		public float m_shakeFrequency = 1.0f;
		...
		protected override void RecacheValues()
		{
			...
			m_iTweenArgs.Set("amount", m_vector);
			m_iTweenArgs.Set("duration", m_duration);
			m_iTweenArgs.Set("shaketype", m_shakeType);
			m_iTweenArgs.Set("frequency", m_shakeFrequency);
			m_iTweenArgs.Set("looptype", m_loopType);
			m_iTweenArgs.Set("loopcount", m_loopCount);
			m_iTweenArgs.Set("delay", m_iTweenDelay);
			m_iTweenArgs.Set("islocal", m_localSpace);
		}
		...
		protected override float PerformAction(bool immediate, bool realTime, ref List<System.UInt64> dependantActionHandles)
		{
			...
			if (m_transform == null)
			{
				DebugHelpers.LogWarningContext("ShakeBy action on \"{0}\" has no Transform specified", this, gameObject.GetNameWithPath());
				return 0.0f;
			}
			...
			{
				// Cancel all Move iTween transitions on this game object. Only one Move can execute at a time
				iTween.StopByType(m_transform.gameObject, "Move", false, true);

				// An instant shake does nothing, because a shake ends up where it starts
				return 0.0f;
			}
			...
			// Execute the iTween
			if (!iTween.ShakePosition(m_transform.gameObject, m_iTweenArgs))
				return 0.0f;
		...
		protected override void CancelAction(bool actionStillRunning, bool actionHasActiveDependants, bool actionListFinished)
		{
			...
			if (actionStillRunning && (m_transform != null))
				iTween.StopByType(m_transform.gameObject, "Move", false, false);
		}
	}
