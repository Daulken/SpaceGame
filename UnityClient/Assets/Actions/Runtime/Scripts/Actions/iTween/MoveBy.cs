...
namespace Actions {
	...
	[AddComponentMenu("SPACEJAM/Actions/Actions/iTween/MoveBy")]
	public class MoveBy : BaseAction
	{
		...
		protected override float PerformAction(bool immediate, bool realTime, ref List<System.UInt64> dependantActionHandles)
		{
			...
			if (m_transform == null)
			{
				DebugHelpers.LogWarningContext("MoveBy action on \"{0}\" has no Transform specified", this, gameObject.GetNameWithPath());
				return 0.0f;
			}
			...
			if ((m_duration <= 0.0f) || immediate || !m_transform.gameObject.activeInHierarchy)
			{
				...
				if (m_localSpace)
					m_transform.localPosition += m_vector;
				else
					m_transform.position += m_vector;
				return 0.0f;
			}
			...
			// Execute the iTween
			if (!iTween.MoveBy(m_transform.gameObject, m_iTweenArgs))
				return 0.0f;
