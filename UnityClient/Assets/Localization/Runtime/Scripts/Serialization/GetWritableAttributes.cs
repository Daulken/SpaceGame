...
namespace Serialization
{
    ...
    public class GetWritableAttributes
    {
        ...
        public static Entry[] GetProperties(object obj, bool seen)
        {
			...
			{
				RadicalLogger.DeferredLog ("[Available Properties]");
				RadicalLogger.IndentDeferredLog();
				accessors[0].Select(a => {  RadicalLogger.DeferredLog ("{0}",a.Info.Name); return a;}).ToList();
        ...
        public static Entry[] GetFields(object obj, bool seen)
        {
			...
			if(RadicalLogger.IsDeferredLogging())
			{
				RadicalLogger.DeferredLog ("[Available Fields]");
				RadicalLogger.IndentDeferredLog();
				accessors[1].Select(a => {  RadicalLogger.DeferredLog ("{0}", a.FieldInfo.Name); return a;}).ToList();
				RadicalLogger.OutdentDeferredLog();
				RadicalLogger.DeferredLog ("[/Available Fields]");
			}
#endif
