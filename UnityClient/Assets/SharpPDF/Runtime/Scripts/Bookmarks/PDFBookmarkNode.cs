...
namespace SharpPDF.Bookmarks {
	...
	public class PDFBookmarkNode : IWritable, IComparable
	{
		...
		private List<PDFBookmarkNode> m_children = new List<PDFBookmarkNode>();		
		...
		/// Bookmark's childs
		/// </summary>
		internal List<PDFBookmarkNode> children
		{
			get
			{
				return m_children;
		...
		public string GetText()
		{
			...
			strBookmark.Append("/Title (" + PDFTextAdapter.CheckText(_Title) + ")" + Convert.ToChar(13) + Convert.ToChar(10));				
			if (_prev > 0)
			{
