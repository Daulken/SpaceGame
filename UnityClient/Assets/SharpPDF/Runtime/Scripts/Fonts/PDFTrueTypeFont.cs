...
namespace SharpPDF.Fonts {
	...
	{
		List<char> _usedCharacters = new List<char>();
		string m_fontResourceName;
		...
		/// <param name="fontResourceName">Font's TextAsset resource name</param>
		internal PDFTrueTypeFont(PDFFontDefinition fontDefinition, int fontNumber, DocumentFontEncoding encodingType, string fontResourceName):
			base(fontDefinition, fontNumber, encodingType)
		{			
			m_fontResourceName = fontResourceName;
		}
		...
		public string GetFontDescendantText()
		{
			...
			{				
				...
				content.Append(getMetric(_usedCharacters[0]).characterMapping.ToString() + "[" + getMetric(_usedCharacters[0]).characterWidth.ToString());
				oldMetric = getMetric(_usedCharacters[0]);
				for (int i = 1; i < _usedCharacters.Count; i++)
				{	
					tempMetric = getMetric(_usedCharacters[i]);
		...
		public string GetToUnicodeText()
		{
			...
			{
				hexContent.Append("<" + getMetric(_usedCharacters[0]).characterMapping.ToString("X4").ToLower() + "><" + getMetric(_usedCharacters[_usedCharacters.Count - 1]).characterMapping.ToString("X4").ToLower() + ">" + Convert.ToChar(13) + Convert.ToChar(10));
			}
			else
			{
			...
			content.Append(PDFTextAdapter.HEXFormatter(hexContent.ToString()) + ">" + Convert.ToChar(13) + Convert.ToChar(10));
		...
		{			
			...
			try
			{
				...
				{
					usedGlyph.Add(getMetric(_usedCharacters[i]).characterMapping, new int[3] {getMetric(_usedCharacters[i]).characterMapping, getMetric(_usedCharacters[i]).characterWidth, _usedCharacters[i]});
				}
				_fontSubset = new SharpPDF.Fonts.TTF.TTFFontSubset(m_fontResourceName, usedGlyph);
		...
		public void AddCharacters(string myText)
		{
			...
			foreach (char myChar in myText.ToCharArray())
			{
				...
				{	
					found = false;
					i = 0;
					//Search the position into the List<char> with the characterMapping Value
					while (i < _usedCharacters.Count && !(found))
					{
						if (getMetric(myChar).characterMapping < getMetric(_usedCharacters[i]).characterMapping)
						{
							found = true;
						}
						else
