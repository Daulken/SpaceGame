...
namespace SharpPDF.Fonts.TTF {
	...
	internal class TTFFontSubset : IDisposable
	{
		...
		public TTFFontSubset(string fontResourceName, Hashtable usedCharacters)
		{
			// Load this as a Unity resource
			m_fontResource = Resources.Load(fontResourceName) as TextAsset;
			if (m_fontResource == null)
				throw new PDFMissingFontFileException(fontResourceName);
			
			_usedGlyphs = usedCharacters;
			_glyphKeys = new List<int>(_usedGlyphs.Keys as ICollection<int>);
		...
		public byte[] GetFontFileStream()
		{
			...
			try
			{
				_fontStream = new AdvancedMemoryStream(m_fontResource.bytes);
				_subsetStream = new AdvancedMemoryStream();
		...
		protected void CheckGlyphComp(int glyph)
		{
			...
			while (!finished)
			{
				...
				if ((flags & (int)GlyphType.MoreComponents) == 0)
					finished = true;
				skip = 0;
				if ((flags & (int)GlyphType.TwoArgs) != 0)
					skip = 4;
				else
					skip = 2;
				if ((flags & (int)GlyphType.SingleScale) != 0)
					skip += 2;
				else if ((flags & (int)GlyphType.DoubleScale) != 0)
					skip += 4;
				if ((flags & (int)GlyphType.TwoByTwo) != 0)
					skip += 8;
