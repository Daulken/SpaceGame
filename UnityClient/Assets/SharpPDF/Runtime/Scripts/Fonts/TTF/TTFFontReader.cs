...
namespace SharpPDF.Fonts.TTF {
	...
	{
		private TextAsset m_fontResource = null;
		private AdvancedMemoryStream _fontStream = null;
		...
		private List<CMAPTable> _CMAPTable = new List<CMAPTable>();
		...
		public TTFFontReader(string fontFilename): base()
		{
			// Load this as a Unity resource
			m_fontResource = Resources.Load(fontFilename) as TextAsset;
			if (m_fontResource == null)
				throw new PDFMissingFontFileException(fontFilename);
			...
			if (_directoryTables.ContainsKey("name"))
			{
				...
				while (i<numTables && !(founded))
				{
					...
					case 6:	//FontName						
						if ((platformID == 0 || platformID == 3 || (platformID == 2 && platformEncodingID == 1)))
						{
							_namesTable.FontName = _fontStream.readUnicodeString(length).Replace(" ", System.String.Empty);
						}
						else
						{
							_namesTable.FontName = _fontStream.readString(length).Replace(" ", System.String.Empty);
						}							
						break;						
					}
		...
		private CMAPTable GetCMAP(int platformID, int encodingID)
		{
			...
			{
				if ((_CMAPTable[i]).PlatformID == platformID && (_CMAPTable[i]).EncodingID == encodingID)
				{
					returnCMAP = _CMAPTable[i];
				}
				i++;
			}
			return returnCMAP;
