...
namespace StarGeneration
{
	public static class StarName
	{
		...
		private static readonly Model MarkovNameModel = new ModelBuilder(3).Teach(RealStarNames).Teach(FictionalLocations).ToModel();
		...
		{
			public float Item1;
			public System.Func<System.Random, string> Item2;

			public Strategy(float i1, System.Func<System.Random, string> i2)
			{
				Item1 = i1;
				Item2 = i2;
			}
		...
		private static readonly List<Strategy> _namingStrategies = new List<Strategy>
			{
		...
		private static readonly List<Strategy> _prefixStrategies = new List<Strategy>
			{
				Weighted( 1.0f, Greek ),
		...
		private static readonly List<Strategy> _suffixStrategies = new List<Strategy>
			{
				Weighted( 1.0f, Greek ),
				Weighted( 1.0f, Decorator ),
		...
		{
			return MarkovNameModel.Generate(random);
		}
		...
		private class WeightedDecoration
		{
			public readonly DecorationType m_type;
			public readonly float m_weight;

			public WeightedDecoration(float weight, DecorationType decoration)
			{
				m_weight = weight;
				m_type = decoration;
			}
		...
		private static Func<Random, string> WithDecoration(float probability, Func<Random, string> func)
		{
			return (r) =>
			{
				...
				switch (
					r.RandomChoice(new List<WeightedDecoration>()
					{
						new WeightedDecoration(0.4f, DecorationType.None),
						new WeightedDecoration(1.0f, DecorationType.Prefix),
						new WeightedDecoration(1.0f, DecorationType.Suffix),
						new WeightedDecoration(0.2f, DecorationType.Both)
					}).m_type)
				{
				case DecorationType.Prefix:
					return prefix + result;
				case DecorationType.Suffix:
					return result + suffix;
				case DecorationType.Both:
		...
		private static Strategy Weighted(float f, Func<Random, string> s)
		{
			return new Strategy(f, s);
		}
		...
		private static System.Func<System.Random, string> WeightedChoice(this Random random, List<Strategy> choices)
		{
		...
		{
			string name = random.WeightedChoice(_namingStrategies)(random);
			while (_generatedNames.Contains(name))
				name = random.WeightedChoice(_namingStrategies)(random);
